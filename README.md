...
- 境界値分析: タスク追加時のバリデーション
  ...
  意図: UI崩れや、タスクを追加するというメイン機能であるため、最も重要な箇所だと考えました
...
- 状態遷移テスト: タスク管理
  ...
  意図：単発の操作ではなく、一連の流れが壊れていないかを検証するため。「削除できない」、「編集できない」という不具合は、ユーザーの使用感に与える影響が大きいと考えました。
...
- 異常系テスト
  ...
  意図: 未ログイン時の不正な操作はアプリの信頼性を失うため。また、不具合が起こりやすい箇所だと考えたため。
